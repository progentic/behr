...
const AUTH_SECRET_VARIABLE = "AUTH_SECRET";
const AUTH_BASE_URL_VARIABLE = "AUTH_BASE_URL";
...
export function loadApiConfig(environment: Environment): ApiConfig {
  ...
  return createApiConfig(port, secret, baseUrl, production);
}
...
function readAuthBaseUrl(environment: Environment): URL {
  const value = requireEnvironmentValue(
    environment,
    AUTH_BASE_URL_VARIABLE,
    "AUTH_BASE_URL is required.",
  );
  const url = parseUrl(value, "AUTH_BASE_URL must be a valid URL.");
  requireHttpProtocol(url);
  requireOriginOnlyUrl(url);
  return url;
}
...
function createApiConfig(
  ...
  return Object.freeze({
    ...
    auth: Object.freeze({
      ...
      trustedOrigins: Object.freeze([origin]),
    }),
  });
}
...
function requireHttpProtocol(url: URL): void {
  if (!HTTP_PROTOCOLS.has(url.protocol)) {
    throw new ApiConfigurationError("AUTH_BASE_URL must use HTTP or HTTPS.");
  }
}

function requireOriginOnlyUrl(url: URL): void {
  ...
  if (hasCredentials || hasExtraParts) {
    throw new ApiConfigurationError("AUTH_BASE_URL must contain only an origin.");
  }
}
